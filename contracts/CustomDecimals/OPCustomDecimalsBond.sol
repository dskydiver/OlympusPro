...
contract OPCustomDecimalsBond is Ownable {
    ...
    uint8 public priceDecimals = 7; // decimal adjustment for price
    uint8 public bcvDecimals = 5; // decimal adjustment for payout
    uint8 public maxPayoutDecimals = 5; // decimal adjustment for max debt
    uint8 public debtRatioDecimals = 0; // decimal adjustment for debt ratio
    ...
    function setDecimals( uint8 _priceDecimals, uint8 _bcvDecimals, uint8 _maxPayoutDecimals, uint8 _debtRatioDecimals ) external onlyPolicy() {
        priceDecimals = _priceDecimals;
        bcvDecimals = _bcvDecimals;
        maxPayoutDecimals = _maxPayoutDecimals;
        debtRatioDecimals = _debtRatioDecimals;
    }
    ...
    function _bondPrice() internal returns ( uint price_ ) {
        price_ = terms.controlVariable.mul( debtRatio() ).div( 10 ** (payoutToken.decimals() - bcvDecimals) );
    ...
    function maxPayout() public view returns ( uint ) {
        return payoutToken.totalSupply().mul( terms.maxPayout ).div( 10 ** maxPayoutDecimals );
    }
    ...
    function payoutFor( uint _value ) public view returns ( uint _payout, uint _fee) {
        if(feeInPayout) {
            uint total = FixedPoint.fraction( _value, bondPrice() ).decode112with18().div( 10 ** (18 - priceDecimals) );
            _fee = total.mul( currentOlympusFee() ).div( 1e6 );
            _payout = total.sub(_fee);
        } else {
            _fee = _value.mul( currentOlympusFee() ).div( 1e6 );
            _payout = FixedPoint.fraction( customTreasury.valueOfToken(address(principalToken), _value.sub(_fee)), bondPrice() ).decode112with18().div( 10 ** (18 - priceDecimals) );
        }
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        ...
        ).decode112with18().div( 10 ** (18 - debtRatioDecimals) );
    }
